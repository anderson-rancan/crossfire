...
for /f %%i in ('git log -1 --pretty=%B') do set commit_msg=%%i
for /f %%i in ('git log -1 --pretty=%an') do set commit_author=%%i
for /f %%i in ('git log -1 --pretty=%ae') do set commit_email=%%i

C:\projects\crossfire\packages\coveralls.net.0.7.0\tools\csmacnz.Coveralls.exe --opencover -i C:\projects\crossfire\coverage.xml --repoToken LNoFBUjtqtwpwK26bCTtuBKuuXgIC45ah --commitId %commit_id% --commitBranch %commit_branch% --commitAuthor %commit_author% --commitEmail %commit_email% --commitMessage %commit_msg%

MSBuild.SonarQube.Runner.exe end /d:"sonar.login=8c036c1a557cea8cfe5550306598d602dbbc5a9b"